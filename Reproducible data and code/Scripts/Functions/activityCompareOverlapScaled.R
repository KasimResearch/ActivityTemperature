...
## FOR TESTING
# 
#  speciesA <- leopardData
#  speciesB <- cheetahData


activityCompareOverlapScaled <- function(speciesA, speciesB) {
...
speciesALow <- as.data.frame(speciesA[speciesA$max_temp24Sc == -2, ])
speciesAMedium <- as.data.frame(speciesA[speciesA$max_temp24Sc == 0, ])
speciesAHigh <- as.data.frame(speciesA[speciesA$max_temp24Sc == 2, ])
...
speciesBMedium <- speciesB[speciesB$max_temp24Sc == 0, ]
