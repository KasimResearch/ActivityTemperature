...
## NEW BIT to make sure fitted values are > 1 but still proportional to one another
speciesA$predictions <- speciesA$predictions*1000
...
speciesB$predictions <- speciesB$predictions*1000
...
for (i in 1:nrow(speciesB)) {
  ...
  # Get the number of 'occurrences'
  nTimes <- as.numeric(round(row[1, 'predictions'])) # FIX BECAUSE IT'S PROPORTIONS THE NUMBER OF OCCURRENCES WILL ALWAYS BE ZERO
